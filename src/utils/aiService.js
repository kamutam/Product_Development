...
function extractDynamicTenderMeta(fileName = '', fileText = '') {
  ...
  // Specific match for slash-separated government reference codes (e.g. GAIL/NDA26028VK/C&P/SECURITY)
  const multiSlashMatch = fileText.match(/\b([A-Z0-9]{2,15}\/[A-Z0-9_\-]{3,30}\/[A-Z0-9&_\-]{2,25}(?:\/[A-Z0-9_\-]{2,25})*)\b/i);

  // Table row match: "TENDER NO. & DATE GAIL/..."
  ...
  if (multiSlashMatch && multiSlashMatch[1] && isValidTenderRef(multiSlashMatch[1])) {
    ...
    if (tenderNoAndDateMatch[2]) {
      tenderDated = tenderNoAndDateMatch[2].trim();
    }
